...
class LEDSearch {
	constructor(options) {
		...
		this.element.classList.add('output__led-search');
		this.searchBar = document.createElement('div');
		this.searchBar.classList.add('output__led-search-bar');
		this.element.appendChild(this.searchBar);
		this.searchInput = document.createElement('input');
		this.searchInput.classList.add('output__led-search-input');
		...
		this.backButton.classList.add('output__led-back');
		...
		this.searchResults.classList.add('output__led-search-results');

		this.allResults = [];
		for (let index = 0; index < this.assets.length; index += 1) {
			let item = document.createElement('div');
			item.classList.add('output__led-search-result');

			let icon = document.createElement('div');
			icon.classList.add('output__led-search-result-icon');
			...
			icon.addEventListener('click', this.displayCallback);
	...
	hide() {
		...
		this.searchResults.className = 'output__led-search-results';
		this.searchInput.className = 'output__led-search-input';
		this.backButton.className = 'output__led-back';

		this.allResults.forEach((item) => {
			let icon = item.children[0];
			icon.className = 'output__led-search-result-icon';
		});
		this.visible = false;
	}
	...
	show(classId) {
		...
		this.searchResults.classList.add(`output__led-search-results--${classId}`);
		this.searchInput.classList.add(`output__led-search-input--${classId}`);
		this.backButton.classList.add(`output__led-back--${classId}`);

		this.allResults.forEach((item) => {
			let icon = item.children[0];
			icon.classList.add(`output__led-search-result-icon--${classId}`);
		});
